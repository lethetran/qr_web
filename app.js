...
function renderCard(item, index){
  ...
  card.innerHTML = `
    <h4 style="color:#ff85a1;margin:0 0 5px">${item.bankRaw}</h4>
    <div style="font-size:13px;color:#636e72">STK: ${item.acc}</div>
    <div style="font-size:12px;margin-bottom:8px">💬 ${des}</div>

    <img src="${item.url}" style="width:100%;max-width:200px;margin:10px auto" />

    <div style="display:flex;gap:8px;justify-content:center;margin-top:10px">
      <button class="btn-ghost" onclick="editItem(${index})">✏️ Sửa</button>
      <button class="btn-secondary" onclick="deleteItem(${index})">🗑 Xoá</button>
      <a href="${item.url}&download=true" target="_blank" class="btn-ghost">⬇ QR</a>
    </div>
  `;
...
function applyFilter(){
  ...
  document.querySelectorAll(".card-qr").forEach(c => {
...
/* ================= EXPORT EXCEL – 1 SHEET + RED ERROR ================= */
